...
class Event extends React.Component {
  static propTypes = {
    ...
    typeOfSurgery: PropTypes.string,
  ...
  stopDrag = (ev) => {
    ev.stopPropagation()
    const { eventResized, _id, operationName, patientName, typeOfSurgery, disabled, dispatch, id, title, startDate, resource, styles } = this.props,
          { width } = this.state,
          newDuration = this.roundToNearest(width)

    dispatch(
      updateEventDuration(
        { disabled, _id, operationName, patientName, typeOfSurgery, id, title, startDate, resource, styles },
        newDuration,
        eventResized
      )
    )
  ...
  render() {
    const { styles, isDragging, connectDragSource, connectDragPreview, id, title, children, rowHeight, ...rest } = this.props,
          ...
          defaultStyles = { color: '#000', backgroundColor: 'CornflowerBlue' },
