...
import PartialEvent from './partial_event.jsx'
...
export default class Chart extends Component {
  ...
  renderEvent(resource, date) {
    ...
    const currentEvent = this.props.events.find(event => {return event.resource === resource && event.startDate === date})
    if (currentEvent) {
      return (
        <Event
          {...currentEvent}
      ...
      const partialEvent = this.props.events.find(event => {
        let eventEnd = new RangeDate(event.startDate).advance('hours', event.duration),
            from = this.props.range.from.date,
            eventStart = new RangeDate(event.startDate).date
        return (
          eventEnd.toRef() === date && from.isAfter(eventStart, 'hours') && event.resource === resource
        )
      })
