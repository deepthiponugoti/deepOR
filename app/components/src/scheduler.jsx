...
import DateRange from './date_range.jsx'
...
export default class Scheduler extends Component {
  ...
  fireEventChanged = (props) => {
    const { onEventChanged } = this.props,
          { id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled } = props
    if (onEventChanged) onEventChanged({ id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled })
  }

  fireEventResized = (props) => {
    const { onEventResized } = this.props,
          { id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled } = props
    if (onEventResized) onEventResized({ id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled })
  }

  fireEventClicked = (props) => {
    const { onEventClicked } = this.props,
          { id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled } = props
    if (onEventClicked) onEventClicked({ id, _id, operationName, patientName, typeOfSurgery, title, startDate, duration, resource, disabled })
  }
